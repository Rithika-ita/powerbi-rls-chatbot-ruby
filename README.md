...
Run `setup.rb` once and it auto-discovers your schema and RLS model — **no code
changes required**.
...
| **Auto-setup** | `setup.rb` discovers your schema, detects RLS roles, and writes config files |
...
| **A published report with RLS** | Any model — setup.rb detects the mapping |
